...
export default function TextEditingController(){
  ...
  useEffect(()=>{
    const run=()=>{
      ...
      const active=project.clips.filter(c=>c.track!=='audio'&&project.playhead>=c.start&&project.playhead<c.start+c.duration)
        .sort((a,b)=>b.trackIndex-a.trackIndex)
      layers.forEach((layer,index)=>{
        const clip=active[index];
        if(!clip||clip.id!==selected)return
        ...
        text.onblur=()=>{
          ...
          if(value) update(clip.id,{name:value})
        }
        text.ondblclick=()=>text.focus()
      })
    }
