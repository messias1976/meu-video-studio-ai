...
function normalizeProject(value: unknown): Project {
  ...
  const tracks: EditorTrack[] = Array.isArray(raw.tracks) && raw.tracks.length
    ? raw.tracks.filter(Boolean).map((t, index): EditorTrack => {
