...
export const useEditorStore = create<EditorState>((set) => ({
  ...
  addClip: assetId => { const a = current.media.find(m => m.id === assetId); if (!a) return; const isAudio = a.kind === 'audio'; const same = current.clips.filter(c => c.track === (isAudio ? 'audio' : 'video')); const start = same.length ? Math.max(...same.map(c => c.start + c.duration)) + 0.2 : 0; const initialDuration = isAudio ? Math.max(.05, a.duration) : Math.max(.05, Math.min(a.kind === 'image' ? 6 : a.duration, 6)); const c: Clip = { id: crypto.randomUUID(), assetId, name: a.name, track: isAudio ? 'audio' : 'video', trackIndex: isAudio ? 3 : same.length % 3, start, duration: initialDuration, x: 50, y: 50, width: 100, height: 100, rotation: 0, opacity: 1, flipX: false, muted: !isAudio, volume: 1 }; const p = { ...current, clips: [...current.clips, c], updatedAt: new Date().toISOString() }; persist(p); set({ project: p, selectedClipId: c.id }) },
