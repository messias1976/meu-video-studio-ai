...
export const useEditorStore = create<EditorState>((set) => ({
  ...
  addClip: assetId => { const a = current.media.find(m => m.id === assetId); if (!a) return; const isAudio = a.kind === 'audio'; const same = current.clips.filter(c => c.track === (isAudio ? 'audio' : 'video')); const start = same.length ? Math.max(...same.map(c => c.start + c.duration)) + 0.2 : 0; const maxInitial = a.kind === 'image' ? 6 : a.duration; const c: Clip = { id: crypto.randomUUID(), assetId, name: a.name, track: isAudio ? 'audio' : 'video', trackIndex: isAudio ? 3 : same.length % 3, start, duration: Math.max(.05, Math.min(maxInitial, 6)), x: 0, y: 0, width: 100, height: 100, rotation: 0, opacity: 1, flipX: false, muted: !isAudio, volume: 1 }; const p = { ...current, clips: [...current.clips, c], updatedAt: new Date().toISOString() }; persist(p); set({ project: p, selectedClipId: c.id }) },
  ...
  setClipTiming: (id, start, duration) => set(s => { const safeStart = Math.max(0, Number(start) || 0); const safeDuration = Math.max(.05, Number(duration) || .05); const p = { ...current, clips: current.clips.map(c => c.id === id ? { ...c, start: safeStart, duration: safeDuration } : c), updatedAt: new Date().toISOString() }; persist(p); return { ...s, project: p } }),
  ...
  duplicateClip: id => set(s => { const source = current.clips.find(c => c.id === id); if (!source) return s; const before = snapshot(); const c: Clip = { ...source, id: crypto.randomUUID(), start: source.start + source.duration + 0.05, name: `${source.name} cópia` }; const p = { ...current, clips: [...current.clips, c], updatedAt: new Date().toISOString() }; persist(p); return { ...s, project: p, selectedClipId: c.id, history: [...s.history, before], future: [], canUndo: true, canRedo: false } }),
