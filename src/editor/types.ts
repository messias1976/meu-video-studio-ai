...
export type Clip = { id:string; assetId:string; name:string; track:Track; trackIndex:number; start:number; duration:number; x:number; y:number; width:number; height:number; rotation:number; opacity:number; flipX:boolean; muted:boolean; volume:number }
