...
.vfs-layer{position:absolute;box-sizing:border-box;overflow:hidden;pointer-events:auto;cursor:move;user-select:none;touch-action:none;transform-origin:center center}
...
.vfs-layer-controls{position:fixed;right:18px;top:88px;width:240px;background:rgba(16,16,20,.97);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:12px;z-index:99999;color:#fff;font:12px Inter,system-ui,sans-serif;box-shadow:0 18px 50px rgba(0,0,0,.45)}
.vfs-layer-controls h4{margin:0 0 8px;font-size:12px}.vfs-layer-controls .vfs-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.vfs-layer-controls label{display:grid;gap:4px;font-size:9px;color:#90909a}.vfs-layer-controls input{width:100%;height:30px;background:#0a0a0d;color:#fff;border:1px solid rgba(255,255,255,.1);border-radius:7px;padding:0 8px;outline:0}.vfs-layer-controls .vfs-row{display:flex;gap:6px;margin-top:8px}.vfs-layer-controls button{flex:1;height:30px;border:1px solid rgba(255,255,255,.1);border-radius:7px;background:#17171d;color:#ddd;cursor:pointer}.vfs-layer-controls button:hover{background:#22222a;color:#fff}.vfs-layer-controls .primary{background:#6d28d9;border-color:#8b5cf6}.vfs-layer-controls small{display:block;color:#686873;margin-bottom:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
function getTransform(c:Clip,index:number):Transform{
 const w=Number.isFinite(c.width)?Number(c.width):(index===0?100:55)
 const h=Number.isFinite(c.height)?Number(c.height):(index===0?100:55)
 return {x:Number.isFinite(c.x)?Number(c.x):(index===0?0:(100-w)/2),y:Number.isFinite(c.y)?Number(c.y):(index===0?0:(100-h)/2),width:w,height:h,rotation:Number(c.rotation||0),flipX:Boolean(c.flipX)}
}
...
function renderControls(project:Project,clip:Clip,t:Transform){
 ...
 el.innerHTML=`<h4>Camada selecionada</h4><small>${assetFor(project,clip.assetId)?.name||'Mídia'}</small><div class="vfs-grid"><label>X (%)<input data-k="x" type="number" step="0.1" value="${t.x.toFixed(1)}"></label><label>Y (%)<input data-k="y" type="number" step="0.1" value="${t.y.toFixed(1)}"></label><label>Largura (%)<input data-k="width" type="number" step="0.1" value="${t.width.toFixed(1)}"></label><label>Altura (%)<input data-k="height" type="number" step="0.1" value="${t.height.toFixed(1)}"></label><label>Rotação (°)<input data-k="rotation" type="number" step="1" value="${t.rotation.toFixed(0)}"></label><label>Camada<input data-k="lane" type="number" step="1" min="0" max="20" value="${clip.lane??0}"></label></div><div class="vfs-row"><button data-action="front" class="primary">Trazer para frente</button><button data-action="back">Enviar para trás</button></div><div class="vfs-row"><button data-action="mirror">Espelhar</button><button data-action="reset">Resetar</button></div>`
 el.querySelectorAll<HTMLInputElement>('input[data-k]').forEach(input=>input.addEventListener('change',()=>{
   const key=input.dataset.k!; const value=Number(input.value); const patch:any={}; if(key==='lane') patch.lane=clamp(Math.round(value),0,20); else patch[key]=value; updateClip(project,clip.id,patch)
 }))
...
function bindLayer(layer:HTMLElement,project:Project,clip:Clip,t:Transform,index:number,stage:HTMLElement){
 layer.addEventListener('pointerdown',(ev)=>{
   ...
   const move=(e:PointerEvent)=>{const d=pctDelta(e.clientX-startX,e.clientY-startY,stage); updateClip(project,clip.id,{x:clamp(startT.x+d.x,0,100-startT.width),y:clamp(startT.y+d.y,0,100-startT.height)})}
 ...
 layer.querySelectorAll<HTMLElement>('.vfs-handle').forEach(handle=>handle.addEventListener('pointerdown',(ev)=>{
   ...
   const move=(e:PointerEvent)=>{
     ...
     if(dir.includes('w')) left=clamp(startT.x+d.x,0,right-5); if(dir.includes('e')) right=clamp(startT.x+startT.width+d.x,left+5,100)
     if(dir.includes('n')) top=clamp(startT.y+d.y,0,bottom-5); if(dir.includes('s')) bottom=clamp(startT.y+startT.height+d.y,top+5,100)
     updateClip(project,clip.id,{x:left,y:top,width:right-left,height:bottom-top})
   }
